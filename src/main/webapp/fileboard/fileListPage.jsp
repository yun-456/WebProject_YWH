    ...
    action속성을 별도로 추가하지 않으면 현재페이지가 된다. -->
					...
					<c:forEach items="${ boardLists }" var="row" varStatus="loop">
						<tr align="center">
							...
							<td align="left">
								...
								href="../fileboard/fileview.do?idx=${ row.idx }">${row.title}</a>
							</td>
							...
							<td><c:if test="${ not empty row.ofile }">
									<a
										href="../fileboard/filedownload.do?ofile=${ row.ofile }&sfile=${row.sfile}&idx=${ row.idx }">[Down]</a>
								</c:if></td>
						</tr>
					</c:forEach>
				</c:otherwise>
		...
		<table>
			<tr align="center">
				...
				<td width="100">
					<button type="button"
						onclick="location.href='../fileboard/filewrite.do';">글쓰기</button>
				</td>
			</tr>
		</table>
		</table>
